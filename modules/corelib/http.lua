-- chunkname: @/modules/corelib/http.lua
...
function HTTP.onPost(operationId, url, err, data)
	...
	if not err and operation.json then
		...
		if type(data) == "table" and not status then
			err = "JSON ERROR: " .. result
...
function HTTP.onDownloadProgress(operationId, url, progress, speed, totalsize)
	local operation = HTTP.operations[operationId]
	...
	if operation.progressCallback then
		operation.progressCallback(progress, speed, totalsize)
	end
end
