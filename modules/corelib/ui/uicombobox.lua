...
function UIComboBox.create()
	...
	combobox.defaultColor = combobox:getColor()
...
function UIComboBox:setCurrentOption(text, dontSignal)
	...
	for i, v in ipairs(self.options) do
		if v.text == text and self.currentIndex ~= i then
			...
			if v.data and v.data.color then
				self:setColor(v.data.color)
			else
...
function UIComboBox:addPlaceholderOption(text)
	table.insert(self.options, {
		placeholder = true,
		text = text
	})
	self:setCurrentOption(text, true)
end
...
function UIComboBox:onMousePress(mousePos, mouseButton)
	...
	for i, v in ipairs(self.options) do
		if not v.placeholder then
			local option = menu:addOption(v.text, function()
				self:setCurrentOption(v.text)
			end)

			if v.data and v.data.icon then
				option:setIcon(v.data.icon)
			end
		end
	end

	menu:setWidth(self.fixedWidth or self:getWidth())
	menu:display({
		x = self:getX(),
