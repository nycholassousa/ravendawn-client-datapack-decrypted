...
function connect(object, arg1, arg2, arg3)
	...
	for signal, slot in pairs(signalsAndSlots) do
		...
		table.insert(disconnectFunctions, function()
			disconnect(object, signalsAndSlots)
		end)
	end

	return disconnectFunctions
end
...
function hasConnect(object, arg1, arg2)
	local signalsAndSlots
	...
	for signal, slot in pairs(signalsAndSlots) do
		local objectSignal = object[signal]

		if objectSignal and type(objectSignal) == "table" then
			for _, func in pairs(objectSignal) do
				if func == slot then
					return true
				end
			end
...
function tr(s, ...)
	local localeModule = g_modules.getModule("client_locale")

	if not localeModule or not localeModule:isLoaded() then
		return string.format(s, ...)
	end

	return modules.client_locale.ClientLocale:translate(s, ...)
end
...
function getFormattedShortNumber(n)
	local ret = tostring(n)

	if n >= 1000000000000 then
		ret = string.format("%.2fT", n / 1000000000000)
	elseif n >= 1000000000 then
		ret = string.format("%.2fB", n / 1000000000)
	elseif n >= 1000000 then
		ret = string.format("%.2fM", n / 1000000)
	elseif n >= 1000 then
		ret = string.format("%.2fK", n / 1000)
	end
...
function format_number(n)
	...
	if n >= 1000000000000 then
		ret = string.format("%.1ft", n / 1000000000000)
	elseif n >= 1000000000 then
		ret = string.format("%.1fb", n / 1000000000)
	elseif n >= 1000000 then
		ret = string.format("%.1fm", n / 1000000)
	elseif n >= 1000 then
		ret = string.format("%.1fk", n / 1000)
	end
