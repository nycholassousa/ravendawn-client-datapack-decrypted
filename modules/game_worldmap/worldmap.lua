...
local mapImageSource = {
	...
	},
	{
		...
		callback = function(lastSource, quality)
			for i = 8, 15 do
				if g_resources.fileExists(string.format("/images/floor%i_%s.jpg", i, quality)) then
					g_textures.preload(string.format("/images/floor%i_%s.jpg", i, quality))
				end
			end
		end
	},
...
function g_worldMap.updateVisibleMarks(updatePosition)
	...
	for _, zoneFlag in pairs(g_worldMap.zoneFlags) do
		if (g_worldMap.iconSettings.showZoneNames or g_worldMap.iconSettings.showLevelBrackets) and (not playerCurrentFloor or playerCurrentFloor <= 7 or zoneFlag.position.z == playerCurrentFloor) then
...
function g_worldMap.init()
	...
	g_keyboard.bindKeyDown("M", g_worldMap.toggle)
...
function g_worldMap.updateImageData(ignoreUpdateVisibleMarks)
	...
	if data then
		...
		if not ignoreUpdateVisibleMarks then
			g_worldMap.updateVisibleMarks()
		end
		...
		if data.tutorial then
			qualityString = txtSize <= 4096 and "l" or "h"
		end
		...
		g_worldMap.image:setImageSource(source)

		local minimap = modules.game_minimap

		minimap.minimapWindow.image:setImageSource(source)
...
function g_worldMap.addFlag(pos, icon, description, temporary, minZoom, maxZoom, callback, profession, data, style, margin, ignoreRaise, hoverCallback)
	...
	minZoom = minZoom or icon == MAPMARK_HOUSE and 3 or 5
...
function g_worldMap.getFlag(pos, icon, description)
	for _, flag in pairs(g_worldMap.flags) do
		if flag.position.x == pos.x and flag.position.y == pos.y and flag.position.z == pos.z and (not icon or icon == flag.icon) and (not description or description == flag.description) then
			return flag
		end
	end
...
function onMenuToggle()
	...
	if player then
		...
		if pos then
			g_worldMap.centerPos.x = (pos.x - g_worldMap.area.fromX) / (g_worldMap.area.toX - g_worldMap.area.fromX)
			g_worldMap.centerPos.y = (pos.y - g_worldMap.area.fromY) / (g_worldMap.area.toY - g_worldMap.area.fromY)
		end

		g_worldMap.destroyHighlights(true)
		modules.game_interface.sendOpcode(ExtendedIds.Regions, {
...
function onConflictIconClicked(widget, regionId, forceUpdate, hovered)
	...
	local type = widget.regionInfo.state .. (g_game.isWarmodeChannel() and "_warmode" or "")
...
function g_worldMap.addCompassData(data, questName, taskId, taskDescription, automatic)
	if data then
		...
		for _, compass in ipairs(data) do
			g_worldMap.displayPositionFromCompass(compass, questName, taskId, taskDescription, automatic)
		end
	end
...
function g_worldMap.displayPositionFromCompass(compass, questName, taskId, taskDescription, automatic, remove)
	...
	if not g_worldMap.getCompassHighlight(pos) then
		g_worldMap.addCompassHighlight(compass, questName, taskId, taskDescription, automatic)
	end
end
...
function g_worldMap.onOutfitChange(player)
	...
	addEvent(function()
		modules.game_minimap.minimapWindow.cross.playerOutfit:setOutfit(parsedOutfit)
	end)
end
