...
function ItemShop:createShopItem(buying, item, index, itemsPanel, quality)
	...
	function widget:enableCheck()
		...
		local selectedQuality = table.findbyfield(ItemsShopQuality, "name", comboBox:getCurrentOption().text)
...
function ItemShop.onOpenNpcTrade(buyCurrency, sellCurrency, items, isCosmetic)
	...
	if not hasItemToSell and table.size(buyItems) == 0 then
		...
		if creature and creature:isNpc() then
			...
			GameChat.onTalk(creature:getName(), nil, TalkTypes.TalkTypeSay, message, nil, creature:getPosition(), creature:getId(), nil, nil, nil)
		end
...
function ItemShop.onBalanceChange(premium, balance)
	ItemShop.window:recursiveGetChildById("silver").value:setText(balance)
end

function ItemShop.onQualityChange(widget)
	local selectedQuality = table.findbyfield(ItemsShopQuality, "name", widget:getCurrentOption().text)
...
function ItemShop.onItemCountChange(widget, text)
	...
	local price = tonumber(widget:getParent().price:getText())
