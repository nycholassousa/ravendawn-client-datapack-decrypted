...
local function resizeToResolution()
	local width = math.min(557, math.max(400, modules.game_interface.getHUDPanel():getWidth() / 3.5))

	GameNpc.window:setWidth(width)
	ItemShop:onResize()
	PortWarehouse:onResize()
	Tradepost:onResize()
	Fishpost:onResize()
end
...
function GameNpc:init()
	...
	connect(HUDPanel, {
		onGeometryChange = resizeToResolution
	})
...
function GameNpc:onGreet(data)
	...
	npcQuestionLabel:setText(npcQuestion.say)
...
function GameNpc:onClose(dontSend, closeGame, hideUngreetMessage)
	...
	modules.game_market.GameMarket:hide()
end
