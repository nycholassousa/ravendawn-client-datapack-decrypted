...
local function moveItemToContainerType(item, containerType)
	if not item:isPickupable() or item:isStackable() then
		return
	end
		...
		if containerType[container:getType()] then
			...
			for slot = 0, container:getCapacity() - 1 do
				if not container:getItem(slot) then
					g_game.move(item, container:getSlotPosition(slot), item:getCount())
...
function UIItem:onItemChange()
	if self.hasBorder then
		...
		local color = ItemQualityColors[qualityName] or ItemQualityColors[quality] or ItemQualityColors[0]

		self:setBorderColor(color)

		local overlayColor = self:getItemId() ~= 0 and self.hasOverlay and (ItemQualityNameOverlayColors[qualityName] or quality ~= ItemQualityNormal and (ItemQualityColors[qualityName] or ItemQualityColors[quality])) or false
