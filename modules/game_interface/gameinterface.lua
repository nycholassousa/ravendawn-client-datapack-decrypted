...
function showSpellCursor()
	...
	gameMapPanel:setCrosshair(spellCursor.widget)
...
function handleCrosshair(ability)
	...
	if spellCursor.spell and spellCursor.spell:lower() == spell.name:lower() and not spellCursor.spell.disableQuickcast then
		...
		if target then
			...
			if tile then
				...
				if protocolGame then
					...
					if position then
						if spellCursor.spell == "fierce leap" then
...
local function onCrosshair(protocol, opcode, buffer)
	...
	function mouseGrabberWidget:onMouseRelease(mousePosition, mouseButton)
		print("mouseGrabberWidget.onMouseRelease")
		...
		if mouseButton == MouseLeftButton then
			...
			if clickedWidget then
				local tile = clickedWidget.getTile and clickedWidget:getTile(mousePosition) or nil
				...
				elseif clickedWidget:getId() == "icon" then
...
function bindKeys()
	...
	for i = 1, #dirs do
		local dir = dirs[i]
		local arr = keys.walk[dir]

		if arr then
			for ii = 1, 3 do
				if arr[ii] and arr[ii] ~= "" then
					bindWalkKey(arr[ii], dir)
				end
			end
		end
...
function canBindWalkKey()
	if GameChat and GameChat.enabled then
		return false
	end
...
function unbindWalkKeys(exceptions)
	for k, v in pairs(keys.walk) do
		for i = 1, 2 do
			if v[i] and v[i] ~= "" then
				local isException = false

				for _, e in pairs(exceptions) do
					if string.find(v[i], e) then
						isException = true
...
function unbindTurnKeys(exceptions)
	for k, v in pairs(keys.turn) do
		for i = 1, 2 do
			if v[i] and v[i] ~= "" then
				local isException = false
...
function bindOptionalWalkKeys(bool)
	for k, v in pairs(keys.optional) do
		for i = 1, #v do
			if v[i] and v[i] ~= "" and table.contains({
				"Q",
				"E",
				"Z",
				"C"
...
function addSelfMenu(menu, creatureThing)
	...
	if not g_game.isInShip() then
		...
		if not localPlayer:isWagonSummoned() and not localPlayer:isWagonToggled() then
	...
	if creatureThing:isPartyMember() and not g_game.isInArenaMode() then
...
function addOtherPlayerMenu(menu, creatureThing)
	...
	if (not localPlayer:isPartyMember() or localPlayer:isPartyLeader()) and not creatureThing:isPartyMember() then
	...
	menu:addOption(tr("Trade with " .. creatureName), function()
	...
	if pos then
		...
		if tile then
			...
			for _, point in ipairs(offsets) do
				...
				if tile then
					local tileCreatures = {
						unpack(tile:getCreatures()),
						unpack(tile:getWalkingCreatures())
					}

					for _, creature in ipairs(tileCreatures) do
						local thingType = creature:getThingType()

						if thingType and not creature:isHidden() then
			...
			for _, creature in ipairs(creatures) do
				if creature.creature ~= g_game.getLocalPlayer() and (creature.distanceX <= creature.width / 2 and creature.distanceY <= creature.height / 2 or g_map.getTile(creature.creature:getPosition()) == tile) then
...
function processMouseAction(menuPosition, mouseButton, autoWalkPos, lookThing, useThing, creatureThing, attackCreature, backpackItem)
	...
	elseif useThing and keyboardModifiers == KeyboardNoModifier and mouseButton == MouseRightButton and not g_mouse.isPressed(MouseLeftButton) then
		...
		elseif useThing:isMultiUse() then
			...
			if useThing and table.find(AbilityBarSupportedItemIds, useThing:getId()) then
				...
				menu:setGameMenu(true)
				menu:addOption("Add to ability bar", function()
	...
	elseif creatureThing and not creatureThing:isHidden() and creatureThing:getPosition().z == autoWalkPos.z and g_keyboard.isAltPressed() and (mouseButton == MouseLeftButton or mouseButton == MouseRightButton) then
		...
		return true
	elseif creatureThing and creatureThing:isPlayer() and keyboardModifiers == KeyboardNoModifier and mouseButton == MouseLeftButton and not g_mouse.isPressed(MouseRightButton) then
		g_game.heal(creatureThing)

		return true
	end

	if autoWalkPos and modules.game_house.GameHouse:checkLandPlotTileDescriptionClick(g_map.getTile(autoWalkPos), menuPosition) then
		return true
	end
...
function removeConfirm(item)
	...
	countWindow = displayGeneralBox(tr("Destroy Item"), string.format("Are you sure you would like to destroy %s%s? This cannot be undone.", itemCount > 1 and itemCount .. "x " or "", item:getName()), {
		{
			text = tr("Yes"),
			callback = yesCallback
		},
...
function UIGameMap.onRegionChange(protocol, opcode, buffer)
	...
	if data.action == "region_change" then
		...
		if data.name then
			GameNotification:displayRegionChange(data.name, data.pvp, data.aboutToPvp, data.timer)
		end
...
function startDecorPlacing(decor)
	...
	gameMapPanel:onMouseMove(g_window:getMousePosition())
	GameNotification:display(NOTIFICATION_INFO, nil, string.format("Choose position to place your %s.%s", decor.name, DECOR_MODELS[decor.id].directions and "\nYou can use your directional keys to change the direction of the decoration." or ""))
	g_mouse.popCursor("target")
	mouseGrabberWidget:ungrabMouse()
...
function showArenaCountdown(count)
