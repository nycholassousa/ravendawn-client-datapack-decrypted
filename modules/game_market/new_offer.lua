...
function GameMarket.onCreateOfferDropItem(widget, droppedWidget)
	...
	itemDropPanel.item_type:setText(string.format("[T%d] %s", tier, itemType))
...
function GameMarket:updateSimilarOffersListPagination()
	...
	self.createOfferWindow.paginationPanel.page_count:setText(string.format("of %i", max_pages), true)
end
...
function GameMarket:selectCreateOfferSelectItem(itemId)
	...
	amountPanel.maximum = not (not item:isStackable() and item:getId() ~= cfg.DUMMY_RAVENCOIN_ITEMID) and 1000 or 1
...
function GameMarket:onCreateBuyOfferAttributesListBoxClick(widget)
	...
	local order = isCraftingAttributeCompatible and {
		"crafting attribute"
	} or cfg.createOfferFiltersOrder
