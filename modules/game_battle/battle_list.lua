...
function BattleList:updateCreatureWidget(widget, creature)
	...
	widget.creatureName:setText(widget.name)

	local stringSize = string.len(widget.name)
	local averageWidthPerChar = widget.creatureName:getTextSize().width / stringSize

	if widget.creatureName:getTextSize().width > widget.creatureName:getWidth() then
		local amountToBeReduced = math.floor((widget.creatureName:getTextSize().width - widget.creatureName:getWidth()) / averageWidthPerChar) + 3

		widget.creatureName:setText(string.format("%s...", widget.name:sub(1, stringSize - amountToBeReduced)))
	end
...
function BattleList:onFilterPanelClick(widget, state)
	...
	for _, filter in ipairs(self.filtersCategories) do
		self.popupMenu:addOption(filter:titleCase(), function(option)
		...
		if filters then
			...
			for _, filterConfig in ipairs(self.filtersForCategory[category]) do
				...
				if filters[filterName] then
					...
					label:setText(string.format("    %s", filterConfig.name))
...
function BattleList:onFilterPanelCheckboxChange(widget, state)
	...
	local changedCategory = selectedCategory:getText():lower()
	local changedFilter = widget:getParent():getText():lower()
...
function BattleList:createExpandedFilterPopup(menu, parentOption, category)
	...
	for _, optionConfig in ipairs(options) do
		...
		self:loadFilterStates(parentOption:getText():lower(), option)
	end
...
function BattleList:loadFilterStates(parentCategory, optionWidget)
	...
	local filterName = optionWidget:getText():lower()
...
function BattleList:doTargetAction(widget, attack)
	...
	if attack and target:isPartyMember() then
		attack = false
	end
	...
	if attack then
		...
		if target:isPartyMember() or target:isInShip() then
			return
		end
...
function BattleList:onUpdateWidgetWidth(widget)
	...
	if widget:getStyleName() == "BattleListCreature" then
		...
		if widget.healthPercent then
			widget.healthBar.health:setPercentage(widget.healthPercent)

			if widget.healthBar.mana:isVisible() then
				widget.healthBar.mana:setPercentage(widget.manaPercent)
			end
		end
	end
end
