...
function GameProfessions:init()
	...
	g_keyboard.bindKeyDown("P", self.toggle)
end
...
function GameProfessions.onProfessionChange(player, professionId, level, percent, oldLevel, oldPercent, exp, reqExp)
	...
	if contentPanel then
		...
		if level ~= oldLevel then
			self:showUnlockedOnly(widget, true, contentPanel.list)
		end
	end
				...
				for _, entry in ipairs(category.category:getChildren()) do
					if entry then
						...
						if levelRequired then
							entry.level:setOn(level >= levelRequired.single)
						end
					end
				end
			end
			...
			for _, data in ipairs(sortedCategories) do
				...
				for index, recipe in ipairs(recipes) do
					...
					if recipe.level then
						entryWidget.level:setText(string.format("Level %s", recipe.level.single))
					end
...
function GameProfessions:selectRecipe(professionId, recipe, isCraftingWindow)
	...
	if recipe.level then
		if recipe.level.single then
			...
			panel.level:setText(string.format("Level required: %i", recipe.level.single))
			panel.level:setColoredText(GetHighlightedText(string.format("{Level required:, #a6aab2} {%i, %s}", recipe.level.single, color)))
		else
			panel.level:hide()
		end
		...
		if recipe.level.mass then
			...
			panel.mass_level:setText(string.format("Mass level required: %i", recipe.level.mass))
			panel.mass_level:setColoredText(GetHighlightedText(string.format("{Mass level required:, #a6aab2} {%i, %s}", recipe.level.mass, color)))
		else
	...
	if recipe.experience then
		desc:setColoredText(GetHighlightedText(string.format("{Experience:, #a6aab2} {%ixp, #36F991}", recipe.experience)))
	...
	for key, material in pairs(materials) do
		if type(key) ~= "string" then
			...
			local list = not (count ~= 1 and key ~= "gathering") and panel or secondaryList
...
function GameProfessions.refreshOwnedMaterials()
	...
	if self.crafting_window.window:isVisible() then
		panel = self.crafting_window.window.info_panel.material_list
...
function GameProfessions.toggle(mouseClick)
	...
	if not self.window:isVisible() and not g_game.isInCutsceneMode() then
		self:show()
	else
		self:hide()
	end
