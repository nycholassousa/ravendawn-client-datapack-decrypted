...
function g_layout.loadMinimap(reset)
	...
	for _, child in pairs(minimap:getChildren()) do
		if child:getStyleName() == "MinimapToggleButton" then
			...
			if buttonData then
				child:setAnglePosition(minimap.image, buttonData.angle, buttonData.additionalDistance)
			end
		end
	end
end
...
function g_layout.loadBanditAvatar(reset)
	...
	banditAvatar:setPosition(data.banditAvatar and data.banditAvatar.position or {
		x = 400,
		y = 40
	})
...
function g_layout.load(reset)
	...
	local chat = GameChat.window

	chat:setPosition(data.console and data.console.position or {
		x = 15,
		y = sHeight - chat:getHeight() - 28
	})
end
	...
	for _, child in pairs(minimap:getChildren()) do
		if child:getStyleName() == "MinimapToggleButton" then
			...
			data.minimap.buttons[child:getId()] = {
				angle = child.angle,
				additionalDistance = child.additionalDistance
			}
		end
	end
end
