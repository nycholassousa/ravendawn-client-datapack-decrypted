		...
		if config then
			if config.type == cfg.PROGRESSION_TYPE_CUMULATIVE then
				for index, reward in ipairs(config.rewards) do
					...
					rewardWidget.description:setText(reward.description)
					...
					dummyWidget.description:setText(reward.description)
				...
				for index, acquire in ipairs(config.acquire) do
					...
					acquireWidget.description:setText(acquire.limit > 1 and string.format("[x%d] %s", acquire.limit, acquire.description) or acquire.description)
			...
			elseif config.type == cfg.PROGRESSION_TYPE_NON_CUMULATIVE then
				for index, reward in ipairs(config.rewards) do
					...
					rewardWidget.text_holder.description:setText(reward.description)
					rewardWidget.icon_holder.icon:setImageSource(string.format("/images/ui/windows/journal/artifacts/small_icons/%s.png", config.artifact_icon or reward.artifact_icon))
					...
					rewardWidget.text_holder.subdescription:setText(config.acquire[index].description)
...
function GameJournal:setArtifactAcquireProgress(regionName, index, remaining, ignoreProgressChange)
	...
	if remaining == 0 then
		if not ignoreProgressChange then
			...
			acquireWidget.progress:setText(string.format("[Completed]", config.description))
	...
	if not ignoreProgressChange then
		...
		acquireWidget.progress:setText(string.format("[%s remaining]", remaining))
		acquireWidget.description:updateSize(true)
	end
end
