...
function GameCharacter:init()
	...
	g_keyboard.bindKeyDown("X", self.toggle)
end
...
function GameCharacter.onGameStart()
	...
	GameCharacter.onCriticalDamageChange(player, player:getCriticalDamageSpell(), player:getCriticalDamageMelee(), player:getCriticalDamageRanged())
	GameCharacter.onCriticalChanceChange(player, player:getCriticalChanceSpell(), player:getCriticalChanceMelee(), player:getCriticalChanceRanged())
...
function GameCharacter.onArchetypeLevelChange(player, id, level, percent)
	...
	addEvent(function()
		GameCharacter.window.panel_top.level_rect:setText(player:getEffectiveLevel())
...
function GameCharacter.onInventoryChange(player, slot, item, oldItem)
	...
	signalcall(g_game.onEquippedItems, player)
...
function GameCharacter.toggle(mouseClick)
...
function GameCharacter:updateOutfitList(outfitCategory, outfits)
	...
	for index, outfit in ipairs(outfits) do
		...
		widget.overlay_text:setText(cfg and cfg.name or "Unknown")
...
function GameCharacter.onCriticalDamageChange(player, spellCriticalDamage, meleeCriticalDamage, rangedCriticalDamage)
	local panel = GameCharacter.window.content.stats_panel.secondary_stats.content
	local widgets = {
		[panel.spell_critical_damage_percent] = spellCriticalDamage,
		[panel.melee_critical_damage_percent] = meleeCriticalDamage,
		[panel.ranged_critical_damage_percent] = rangedCriticalDamage
	}
...
function GameCharacter.onCriticalChanceChange(player, spellCriticalChance, meleeCriticalChance, rangedCriticalChance)
	local panel = GameCharacter.window.content.stats_panel.secondary_stats.content
	local widgets = {
		[panel.spell_critical_chance_percent] = spellCriticalChance,
		[panel.melee_critical_chance_percent] = meleeCriticalChance,
		[panel.ranged_critical_chance_percent] = rangedCriticalChance
	}
