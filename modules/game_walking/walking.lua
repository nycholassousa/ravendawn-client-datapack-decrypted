...
function isWalkKeyPressed(dir)
	local arr = modules.game_interface.keys.walk[dir]

	if arr then
		for i = 1, 3 do
			if arr[i] and g_keyboard.isKeyPressed(arr[i]) then
				return true
			end
		end
	end
...
function walk(dir, ticks)
	...
	if g_game.isInCutsceneMode() or player:isDiagonalWalkLocked() and dir > West then
		nextWalkDir = nil
