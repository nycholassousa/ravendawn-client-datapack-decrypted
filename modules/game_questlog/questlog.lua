...
function GameQuestLog:init()
	...
	g_keyboard.bindKeyDown("L", self.toggle)
...
function GameQuestLog.toggle(mouseClick)
	...
	if not GameQuestLog.window:isVisible() and not g_game.isInCutsceneMode() then
		GameQuestLog.window.content.completedquests_panel.panel:destroyChildren()
		GameQuestLog.window.content.currentquests_panel.panel:destroyChildren()
...
function GameQuestLog.onQuestDetails(data)
	...
	if player then
		...
		content.descriptionPanel.recommendedLevel:setColoredText(GetHighlightedText(string.format("Level {[%i], %s}", data.level, levelColor)), "white")
	end
...
function GameQuestLog:onRangersCompanyQuestDetails()
	...
	if player then
		...
		content.descriptionPanel.recommendedLevel:setColoredText(GetHighlightedText(string.format("Level {[%i-%i], %s}", data.level.min, data.level.max, levelColor)), "white")
	end
...
function GameQuestLog:onNorthStarQuestDetails()
	...
	if player then
		...
		content.descriptionPanel.recommendedLevel:setColoredText(GetHighlightedText(string.format("Legacy Level {[%i-%i], %s}", data.level.min, data.level.max, levelColor)), "white")
	end
