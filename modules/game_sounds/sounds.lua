...
function g_sound.init()
	if not g_sounds or g_sounds.initialized or g_wwise == nil then
		return
	end
...
function g_sound.online()
	...
	g_sound.playlistMusic = g_sound.play(EVENT_MUSIC_PLAYLIST_TEMPORARY)
...
function g_sound.onShipChange(player, ship, windDirection)
	if ship then
...
function g_sound.play(eventId, emitterId, pos, maxVolume, stepChange, volumeBoost, isAmbientSund)
	...
	if not g_sound.isLocalEmitter(emitterId) then
		if emitterId and emitterId > 0 then
			...
			if not creature then
				...
				if not isAmbientSund and disconnects then
					scheduleEvent(function()
...
function g_sound.tryUnloadSoundbanks()
	for soundbank, loadedTime in pairs(g_sound.loadedSoundbanks) do
		if os.time() - loadedTime > 60 and soundbank ~= "trailer.bnk" and soundbank ~= "main.bnk" and not soundbank:find("music", 1, true) and not soundbank:find("ambience", 1, true) then
