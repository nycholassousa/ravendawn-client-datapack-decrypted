...
function init()
	connect(LocalPlayer, {
		...
		onArchetypeResourceChange = onArchetypeResourceChange,
	...
	archetypes.slots[1].widget.setPercentage = function(self, value, maxValue)
		...
		elseif self.value == 0 then
			...
			function self.animation_func()
				bar:setImageSource(string.format("%s/aether_animation/%d/%s.png", resourceBarAssetPrefix, self.value, string.format("%02d", self.animation_frame)))
...
function activateArchetype(archetypeId)
	...
	if slot.widget then
		...
		if localPlayer then
			local resourceCount = localPlayer:getResourceCount()
			local maxResourceCount = localPlayer:getMaxResourceCount()

			slot.widget:setPercentage(resourceCount, maxResourceCount)
		end
	end
end
...
function onExperienceChange(localPlayer, experience, oldExp, nextLevelExp)
	...
	if localPlayer:getExperienceDebt() == 0 then
...
function onOutfitChange(localPlayer, outfit)
	...
	mountHud.staminaBar.mount_icon:setOutfit({
...
function onMountStaminaChange(localPlayer, stamina, maxStamina)
	...
	mountHud.conditions.hungry:setVisible(stamina == 0)
end
...
function onPartyAddMember(name, isOnline, healthPercent, manaPercent, position, channelId, outfit, temporaryOutfit)
	...
	if isOnline then
		...
		modules.game_minimap.onPositionChange(g_game.getLocalPlayer(), position, position)
		widget.health:setPercentage(widget.healthPercent)
		widget.mana:setPercentage(widget.manaPercent)
		...
		widget:disable()
		widget.health:setPercentage(100)
		widget.mana:setPercentage(100)
	end
...
function onPartyChangePosition(name, position)
	...
	flag.pos = {
		x = (position.x - g_worldMap.area.fromX) / (g_worldMap.area.toX - g_worldMap.area.fromX),
...
function onPartyChangeState(name, isOnline)
	...
	if isOnline then
		widget:enable()
		widget:setPercentage(widget.healthPercent)
...
function updateExperienceBarTooltip(data)
	addEvent(function()
		...
		local text = string.format("Legacy Level %d [%d%%] - %d/%d\nDeath Debt - %d", player:getLevel(), player:getLevelPercent(), player:getExperience(), player:getNextLevelExp(), player:getExperienceDebt())
		...
		experienceBar.experienceTooltip = {
			header = {
				level = GetHighlightedText(string.format("{Legacy Level %d, #FFA851} {[%d%%], white} - %d/%d", player:getLevel(), player:getLevelPercent(), player:getExperience(), player:getNextLevelExp()), "#CED2D9"),
				death_penalty = string.format("Death Penalty - %d", player:getExperienceDebt())
			},
			body = {
