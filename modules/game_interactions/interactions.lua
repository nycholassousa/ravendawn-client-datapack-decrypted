...
function GameInteractions:init()
	...
	function self.action_window.show()
		...
		self.action_window:setVisible(true)
	end
	...
	function self.description_window.show()
		...
		self.description_window:setVisible(true)
	end
...
function GameInteractions:displayDescriptionBox(data, forcePage)
	...
	if #data.pages > 1 then
		button_panel.page_text:setText(string.format("%i of %i", 1, #data.pages))
	...
	window.bandit_avatar:setVisible(data.bandit)
...
function GameInteractions:changeDescriptionBoxPage(direction, userInput)
	...
	button_panel.page_text:setText(string.format("%i of %i", nextPage, #pages))
...
function GameInteractions:displayActionBox(data)
	...
	for i, key in ipairs(data.keys) do
		...
		if i < #data.keys then
			local separator = g_ui.createWidget("GameInteractionsKeySpacer", keys_panel)
		end
	end
...
function GameInteractions:advanceInteraction(env, currentTask, currentInteraction, args)
	...
	if env.tasks[env.current_task] and env.tasks[env.current_task][env.current_interaction] then
...
function GameInteractions:loadInteractionProgress(env)
	...
	if env.tasks and env.tasks[env.current_task] and env.tasks[env.current_task][env.current_interaction] then
...
function GameInteractions:updateBanditAvatarLayoutChange()
	...
	self.bandit_avatar:setVisible(not isEditMode and self.wasBanditVisibleBeforeLayoutChange or isEditMode)
...
function GameInteractions:checkCustomInteractionTriggers()
	if self.bandit_avatar.shakeEvent then
		removeEvent(self.bandit_avatar.shakeEvent)
	...
	if self.custom_interactions.active then
		self.bandit_avatar:setVisible(true)
...
function GameInteractions:configureListWindow()
	...
	if self.custom_interactions.available then
		for interactionName, data in pairs(self.custom_interactions.available) do
			if not data.hide then
				...
				widget.name:setText(interactionName:titleCase())
				widget.name:update()
		...
		table.sort(options, function(a, b)
			return self.custom_interactions.available[a.name:getText():lower()].index < self.custom_interactions.available[b.name:getText():lower()].index
		end)
	end
	...
	if anyStarted then
		...
		widget.name:setText(self.custom_interactions.active.name:titleCase())
		widget.name:update()
	...
	for _, child in ipairs(self.list_window.content:getChildren()) do
		if anyStarted and child.name:getText():lower() == self.custom_interactions.active.name:lower() then
...
function GameInteractions:cancelInteraction(name)
	...
	self.confirmation_box = displayGeneralBox(tr("Cancel Interaction"), "Are you sure you want to cancel this interaction?", {
		{
			text = tr("Yes"),
